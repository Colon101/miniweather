...
function getDayOfWeek(num: number): ForecastResult['dayName'] {
	const daysOfWeek: ForecastResult['dayName'][] = [
		'Sunday',
		'Monday',
		'Tuesday',
		'Wednesday',
	...
	try {
		if (ipAddress.startsWith('127.0.0')) {
			ipAddress =
				...
				event.request.headers.get('x-forwarded-ip') ||
				null;
			if (ipAddress == null) {
		...
		if (!!ipAddress) {
			...
			if (cache.has(ipAddress)) {
				...
				const weatherResponse: Response = await fetch(
					`https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${data.lon},${data.lat}?key=${apikey}&unitGroup=metric`
				);
